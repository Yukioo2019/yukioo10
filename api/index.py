...
class GameInfo:
    def __init__(self):
        self.TitleId: str = ""
        self.SecretKey: str = ""
        self.DiscordWebhook: str = "cool"
        ChangeGorilla: str = "depreso" # idk if work this should change gorilla#### to what ever u put in here
...
def main():
    return "thanks tictac edited by depreso"
...
def validate_nonce(user_id: str, nonce: str):
    ...
    def get_valid(url1, url2, url3) -> bool:
        try:
            ...
            return False # put as what ever u want / true or false to enable
...
def consume_oculus_iap():
    ...
    response = requests.post(
        ...
        url=f"https://graph.oculus.com/consume_entitlement?nonce={nonce}&user_id={user_id}&sku={sku}&access_token={settings.ApiKey2}",
        url=f"https://graph.oculus.com/consume_entitlement?nonce={nonce}&user_id={user_id}&sku={sku}&access_token={settings.ApiKey3}",
        headers={"content-type": "application/json"},
    )
...
def discord_message(message):
  ...
  headers = {'Content-Type': 'application/json'}
  requests.post(f"{DiscordWebhook}", json=payload, headers=headers)
